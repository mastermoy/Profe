...
public class UserData {
    ...
    {        
       CallableStatement query = Conectar.prepareCall("{call sp_insertUser(?,?,?,?,?,?)}");
    ...
    public static List<eUsers> getUsers(Connection conectar) throws Exception{
       List<eUsers> lUsuarios = new ArrayList<>();
        try {
            CallableStatement query = conectar.prepareCall("{call sp_getUsers()}");
            ResultSet resultset = query.executeQuery();
    ...
    public static boolean deleteUser(String userName, Connection conectar) throws Exception{
        ...
        try {
            CallableStatement query = conectar.prepareCall("{call sp_deleteUser(?)}");
    ...
    {        
       CallableStatement query = Conectar.prepareCall("{call sp_updateUser(?,?,?,?,?,?,?)}");
